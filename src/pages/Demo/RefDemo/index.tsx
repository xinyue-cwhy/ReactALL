  ...
  return (
    <Card title="useRef 操作 DOM" size="small" style={{ borderRadius: 8 }}>
      <Space direction="vertical" style={{ width: '100%' }}>
        <input
          ref={inputRef}
          defaultValue="通过 ref 直接操作 DOM 元素"
          style={{
            width: '100%',
        ...
        />
        <Space>
          <Button size="small" onClick={focus}>
            focus()
          </Button>
          <Button size="small" onClick={blur}>
            blur()
          </Button>
...
const ForwardChild = forwardRef<ChildHandle, { label: string }>(
  (props, ref) => {
    ...
    return (
      <Card
        size="small"
        title={props.label}
        style={{ border: '2px dashed #1677ff', borderRadius: 8 }}
      >
        <input
          ref={inputRef}
          value={value}
      ...
      <Space direction="vertical" style={{ width: '100%' }}>
        ...
        <Space wrap>
          <Button
            size="small"
            type="primary"
            onClick={() => childRef.current?.setFocus()}
          >
            调用 setFocus()
          </Button>
          <Button
        ...
        {parentMsg && (
          <Alert
            message={
              <>
                父组件读取到子组件的值: <Text strong>{parentMsg}</Text>
              </>
            }
            type="success"
            showIcon
  ...
  return (
    ...
    >
      <Space direction="vertical">
        ...
        <Text type="secondary">
          timerRef.current 存储定时器 ID，修改它不会触发重渲染
        </Text>
      </Space>
    </Card>
  )
