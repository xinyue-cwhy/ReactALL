...
import { Card, Col, Row, Typography, Tag, Button, Space, Divider } from 'antd'
import {
  NodeIndexOutlined,
  GlobalOutlined,
  ThunderboltOutlined,
...
const Home: React.FC = () => {
  ...
  return (
    <div>
      ...
      <Paragraph>
        本项目使用 <Text code>React 18</Text> + <Text code>Vite</Text> + <Text code>TypeScript</Text> 搭建，
        涵盖 React 全家桶核心技术栈，演示所有组件通信方式。
      </Paragraph>

      <Divider>技术栈</Divider>
      <Space wrap style={{ marginBottom: 24 }}>
        {['React 18', 'Vite 5', 'TypeScript', 'React Router v6', 'Redux Toolkit', 'Zustand',
          'TanStack Query', 'Axios', 'Ant Design 5', 'mitt'].map((t) => (
          ...
          <Col xs={24} sm={12} lg={8} key={d.key}>
            ...
            >
              <Space direction="vertical" style={{ width: '100%' }}>
                ...
                <Paragraph type="secondary" style={{ marginBottom: 8 }}>{d.desc}</Paragraph>
                <Space wrap>
                  {d.tags.map((tag) => <Tag key={tag}>{tag}</Tag>)}
                </Space>
                <Button type="primary" ghost size="small" onClick={() => navigate(d.key)}>
                  查看演示
                </Button>
              </Space>
            </Card>
